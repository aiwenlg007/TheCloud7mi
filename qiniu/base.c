...
{ 
	char * pSzDebugLogString = (char*)malloc(1024);
	memset(pSzDebugLogString,0,1024);
	//va_start(args->items, fmt);

	_vsnprintf(pSzDebugLogString, 1023,fmt, args->items);
	va_end(args->items);
	{
		printf("str:%s\n",pSzDebugLogString);
	} 

	//Qiniu_Buffer_Init(self, 1024);
	Qiniu_Buffer_Write(self, pSzDebugLogString, strlen(pSzDebugLogString));
}

void Qiniu_Buffer_AppendFormatV___old(Qiniu_Buffer* self, const char* fmt, Qiniu_Valist* args)
{
	unsigned char ch;
	const char* p;
